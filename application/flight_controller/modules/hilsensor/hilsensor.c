				     ...
				     struct sensor_value *val)
{
	...
	switch (chan) {
		case SENSOR_CHAN_ACCEL_X:
			return sensor_value_from_float(val, data->hil_data.ax);
		case SENSOR_CHAN_ACCEL_Y:
			return sensor_value_from_float(val, data->hil_data.ay);
		case SENSOR_CHAN_ACCEL_Z:
			return sensor_value_from_float(val, data->hil_data.az);
		case SENSOR_CHAN_GYRO_X:
			return sensor_value_from_float(val, data->hil_data.gx);
		case SENSOR_CHAN_GYRO_Y:
			return sensor_value_from_float(val, data->hil_data.gy);
		case SENSOR_CHAN_GYRO_Z:
			return sensor_value_from_float(val, data->hil_data.gz);
		case SENSOR_CHAN_PRESS:
			return sensor_value_from_float(val, data->hil_data.p);
		default:
			return -ENOTSUP;
	}
	return 0;
